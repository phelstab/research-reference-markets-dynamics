...
class NewMomentumAgent(NewTradingAgent):
    """
    ...
    def place_orders(self, bid: int, ask: int) -> None:
        ...
        if bid and ask:
            ...
            if len(self.mid_list) > 20:
                self.avg_20_list.append(
                    NewMomentumAgent.ma(self.mid_list, n=20)[-1].round(2)
                )
            if len(self.mid_list) > 50:
                self.avg_50_list.append(
                    NewMomentumAgent.ma(self.mid_list, n=50)[-1].round(2)
                )
        ...
        # check fixed size fee
        # check variable size fee 
        # create logic for order type (market order or limit order), and add fees to order
            # if variable/fixed size fee is greater than maker taker fee, then use market order
            # if variable/fixed size fee is less than maker taker fee, then use limit order
                # update order size to account for fees and check behaivour again
        # substract fee from cash
        # log the fee for calculation plotting
