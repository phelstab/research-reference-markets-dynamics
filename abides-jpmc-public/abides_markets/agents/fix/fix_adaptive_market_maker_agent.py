...
class FixAdaptiveMarketMakerAgent(FixTradingAgent):
    """
    ...
    def place_orders(self, mid: int) -> None:
        ...
        if self.backstop_quantity != 0:
            ...
            orders.append(
                self.create_limit_order(
                    self.symbol, self.backstop_quantity, Side.BID, bid_price, order_fee=Fees.get_fixed_market_fee(self) * self.backstop_quantity,
                )
            )
            ...
            orders.append(
                self.create_limit_order(
                    self.symbol, self.backstop_quantity, Side.ASK, ask_price, order_fee=Fees.get_fixed_market_fee(self) * self.backstop_quantity,
                )
            )
        ...
        for bid_price in bid_orders:
            ...
            orders.append(
                self.create_limit_order(
                    self.symbol, self.buy_order_size, Side.BID, bid_price, order_fee=Fees.get_fixed_market_fee(self) * self.buy_order_size,
                )
            )
